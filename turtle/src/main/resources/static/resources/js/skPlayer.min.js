		...
		var s = {
			...
			l = function() {
				function t(e) {
					...
					if (this.option = e, !(this.option.music && this.option.music.type && this.option.music.source)) return console
						.error("请正确配置对象！"), Object.create(null);
					this.root = this.option.element, this.type = this.option.music.type, this.music = this.option.music.source,
						...
						this.type && (this.root.innerHTML = '<p class="skPlayer-tip-loading">LOADING</p>', s.ajax({
				...
				return r(t, [{
						...
						this.dom = {
							...
							.className = "skPlayer-curMusic", "cloud" === this.type && s.ajax({
								...
								success: function(e) {
									...
									null !== n ? (console.log("歌曲拉取成功！"), t.audio.src = n) : (console.log("歌曲拉取失败！ 资源无效！"), 1 !== t.music
										.length && t.next())
								},
								fail: function(t) {
					...
					value: function(t) {
						var e = this;
						return "number" != typeof t ? void console.error("请输入正确的歌曲序号！") : (t -= 1) < 0 || t >= this.music.length ?
							void console.error("请输入正确的歌曲序号！") : t == this.dom.musiclist.querySelector(".skPlayer-curMusic").getAttribute(
								...
								this.music[t].name, this.dom.author.innerHTML = this.music[t].author, this.dom.cover.src = this.music[t]
								.cover, void("file" === this.type ? (this.audio.src = this.music[t].src, this.play()) : "cloud" ===
									this.type && s.ajax({
										...
										success: function(t) {
											...
											null !== n ? (console.log("歌曲拉取成功！"), e.audio.src = n, e.play()) : (console.log("歌曲拉取失败！ 资源无效！"),
					...
					key: "play",
					value: function() {
						this.audio.paused && (this.audio.play(), this.dom.playbutton.classList.add("skPlayer-pause"), this.dom.cover
							.classList.add("skPlayer-pause"))
					}
				}, {
					key: "pause",
					value: function() {
						this.audio.paused || (this.audio.pause(), this.dom.playbutton.classList.remove("skPlayer-pause"), this.dom
					...
					key: "toggleList",
					value: function() {
						this.root.classList.contains("skPlayer-list-on") ? this.root.classList.remove("skPlayer-list-on") : this.root
							.classList.add("skPlayer-list-on")
					}
				}, {
					key: "toggleMute",
					value: function() {
						this.audio.muted ? (this.audio.muted = !1, this.dom.volumebutton.classList.remove("skPlayer-quiet"), this
							.dom.volumeline_value.style.width = s.percentFormat(this.audio.volume)) : (this.audio.muted = !0, this.dom
